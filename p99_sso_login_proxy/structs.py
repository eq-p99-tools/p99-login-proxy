...
class FirstFrag(ctypes.Structure):
    _pack_ = 1
    _fields_ = [
        ("protocol_opcode", ctypes.c_ushort),
...
class Frag(ctypes.Structure):
    _pack_ = 1
    _fields_ = [("protocol_opcode", ctypes.c_short), ("sequence", ctypes.c_short)]
...
def get_protocol_opcode(data: bytes):
    ...
    logger.debug("get_protocol_opcode: %d (%s)", opcode, OPCodes(opcode).name)
